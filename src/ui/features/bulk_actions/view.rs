...
pub fn view<'a>(
    ...
    let clear_btn = button(text("Clear").size(12).color(p.text_secondary))
        .style(theme::ghost_button)
        .padding([6, 10])
        .on_press_maybe((selection_count > 0).then_some(BulkActionMessage::Clear));
    ...
    .style(theme::ghost_button)
    .padding([6, 10])
    .on_press(BulkActionMessage::ToggleMode);
    ...
    .padding([6, 12])
    .on_press_maybe((selection_count > 0).then_some(BulkActionMessage::MarkAsRead));
    ...
    .padding([6, 12])
    .on_press_maybe((selection_count > 0).then_some(BulkActionMessage::MarkAsDone));
